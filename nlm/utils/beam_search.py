...
def compute_topk_scores_and_seq(sequences, scores, scores_to_gather, flags,
  ...
  """
  _, topk_indexes = tf.nn.top_k(scores, k=beam_size) # AMBYER >> get top from the last dimension, scores <batch, 2 * beam>; topk_indexes <batch, beam>
  ...
  topk_seq = tf.gather_nd(sequences, top_coordinates) # AMBYER >> gather_nd : from sequences extract the slices, and the indices is defined in top_coordinates
...
def beam_search(symbols_to_logits_fn,
  ...
  """
  batch_size = tf.shape(initial_ids)[0] # AMBYER >> decode_batch = batch

  # Assume initial_ids are prob 1.0
  initial_log_probs = tf.constant([[0.] + [-float("inf")] * (beam_size - 1)]) # AMBYER >> <1, beam_size>
  # Expand to beam_size (batch_size, beam_size)
  alive_log_probs = tf.tile(initial_log_probs, [batch_size, 1]) # AMBYER >> <batch, beam>

  # Expand each batch to beam_size
  alive_seq = tf.tile(tf.expand_dims(initial_ids, 1), [1, beam_size]) # AMBYER >> <batch, beam>
  alive_seq = tf.expand_dims(alive_seq, 2)  # (batch_size, beam_size, 1) # AMBYER >> <batch, beam, 1>

  # Finished will keep track of all the sequences that have finished so far
  # Finished log probs will be negative infinity in the beginning
  # finished_flags will keep track of booleans
  finished_seq = tf.zeros(tf.shape(alive_seq), tf.int32) # AMBYER >> <batch, beam, 1>
  ...
  def grow_finished(finished_seq, finished_scores, finished_flags, curr_seq,
    ...
    finished_seq = tf.concat(
        [finished_seq,
         tf.zeros([batch_size, beam_size, 1], tf.int32)], axis=2) # TODO : AMBYER >> make the same length with finished scores??

    # Set the scores of the unfinished seq in curr_seq to large negative
    # values
    curr_scores += (1. - tf.to_float(curr_finished)) * -INF # AMBYER >> set the lived one to -inf to mask it
  ...
  def grow_alive(curr_seq, curr_scores, curr_log_probs, curr_finished):
    ...
    # values
    curr_scores += tf.to_float(curr_finished) * -INF # AMBYER >> the finished seq's score is -inf
  ...
  def grow_topk(i, alive_seq, alive_log_probs):
    ...
    flat_ids = tf.reshape(alive_seq, [batch_size * beam_size, -1]) # AMBYER >> <batch, beam, i> -> <batch * beam, i>

    # (batch_size * beam_size, decoded_length)
    flat_logits = symbols_to_logits_fn(flat_ids) # AMBYER >> flat_ids means that let batch * beam be the 'batch'
    logits = tf.reshape(flat_logits, (batch_size, beam_size, -1)) # AMBYER >> <batch, beam, vsize>

    # Convert logits to normalized log probs
    candidate_log_probs = log_prob_from_logits(logits) # AMBYER >> <batch, beam, vsize>

    # Multiply the probabilites by the current probabilites of the beam.
    # (batch_size, beam_size, vocab_size) + (batch_size, beam_size, 1)
    log_probs = candidate_log_probs + tf.expand_dims(alive_log_probs, axis=2)# AMBYER >> <batch, beam, vsize> + <batch, beam, 1> add the current word's score to the sent's score

    length_penalty = tf.pow(((5. + tf.to_float(i + 1)) / 6.), alpha)

    curr_scores = log_probs / length_penalty # AMBYER >> <batch, beam, vsize>
    # Flatten out (beam_size, vocab_size) probs in to a list of possibilites
    flat_curr_scores = tf.reshape(curr_scores, [-1, beam_size * vocab_size]) # AMBYER >> <batch, beam * vsize>

    topk_scores, topk_ids = tf.nn.top_k(flat_curr_scores, k=beam_size * 2) # TODO(beam * 2 -> see upper information) AMBYER >> top_scores <batch, beam * 2>

    # Recovering the log probs becuase we will need to send them back
    topk_log_probs = topk_scores * length_penalty # <batch, beam * 2>

    # Work out what beam the top probs are in.
    topk_beam_index = topk_ids // vocab_size # <batch, beam * 2>
    topk_ids %= vocab_size  # Unflatten the ids # <batch, beam * 2>
    ...
    # stacking will create a tensor of dimension batch * beam * 2, where the
    # last dimension contains the i,j gathering coordinates.
    topk_coordinates = tf.stack([batch_pos, topk_beam_index], axis=2) # AMBYER >> batch_pos <batch, 2 * beam>, topk_beam_index <batch, 2 * beam>, after stack <batch, 2 * beam, 2>

    # Gather up the most probable 2*beams both for the ids and finished_in_alive
    # bools
    topk_seq = tf.gather_nd(alive_seq, topk_coordinates) # AMBYER >> alive_seq <batch, beam, i>, topk_coordinates <batch, beam * 2, 2> -> <batch, beam * 2, i>

    # Append the most probable alive
    topk_seq = tf.concat([topk_seq, tf.expand_dims(topk_ids, axis=2)], axis=2) # AMBYER >> topk_seq <batch, beam * 2, i + 1>

    topk_finished = tf.equal(topk_ids, eos_id) # AMBYER >> check the finished one, topk_finished <batch, 2 * beam>

    return topk_seq, topk_log_probs, topk_scores, topk_finished # AMBYER >> <batch, beam * 2, i + 1>, <batch, beam * 2>, <batch, beam * 2>, <batch, beam * 2>

  def inner_loop(i, alive_seq, alive_log_probs, finished_seq, finished_scores,
                 finished_flags, attarr): # add by ambyer
    ...
    alive_seq, alive_log_probs, _ = grow_alive(topk_seq, topk_scores,
                                               topk_log_probs, topk_finished) # AMBYER >> output's shape <batch, beam, i>, <batch, beam>, <batch, beam>
    ...
    new_attarr = attarr.write(i, tf.get_default_graph().get_tensor_by_name("while/body/model/parallel_0/body/decoder/layer_5/encdec_attention/dot_product_attention/attention_weights:0")[:,:,-1,:]) # add by ambyer
    return (i + 1, alive_seq, alive_log_probs, finished_seq, finished_scores,
            finished_flags, new_attarr)

  def _is_finished(i, unused_alive_seq, alive_log_probs, unused_finished_seq,
                   finished_scores, finished_in_finished, attarr): # add by ambyer
    ...
    return tf.logical_and(tf.less(i, decode_length), tf.logical_not(bound_is_met)) # modified by ambyer

  initArr = tf.TensorArray(size = 0, dynamic_size = True, dtype = tf.float32, name = 'att_init_arr') # add by ambyer
  (tlen, alive_seq, alive_log_probs, finished_seq, finished_scores,
   finished_flags, finallAttarr) = tf.while_loop( # add by ambyer
       _is_finished,
       inner_loop, [
           tf.constant(0), alive_seq, alive_log_probs, finished_seq,
           finished_scores, finished_flags, initArr  # add by ambyer
       ],
       shape_invariants=[
           tf.TensorShape([]),
           ...
           tf.TensorShape([]) # add by ambyer
       ],
       parallel_iterations=1,
       back_prop=False)

  attWeights = finallAttarr.stack(name = 'my_final_attarr') # add by ambyer
