...
def add_timing_signal_1d(x, min_timescale=1.0, max_timescale=1.0e4):
  ...
  """
  length = tf.shape(x)[1] # AMBYER >> positional embedding, x's shape <batch, len, dim>
  channels = tf.shape(x)[2] # AMBYER >> dim
  position = tf.to_float(tf.range(length)) # AMBYER >> [0., 1., 2., ..., len.]
  num_timescales = channels // 2 # AMBYER >> dim // 2
  log_timescale_increment = (math.log(float(max_timescale) / float(min_timescale)) /(tf.to_float(num_timescales) - 1)) # AMBYER >> log_timescale_increment is a scalar
  inv_timescales = min_timescale * tf.exp(tf.to_float(tf.range(num_timescales)) * -log_timescale_increment) # AMBYER >> inv_timescales shape <dim // 2>
  scaled_time = tf.expand_dims(position, 1) * tf.expand_dims(inv_timescales, 0) # AMBYER >> scaled_time shape <len, dim // 2>
  signal = tf.concat([tf.sin(scaled_time), tf.cos(scaled_time)], axis=1) # AMBYER >> concate(<len, dim // 2>, <len, dim // 2>) = <len, dim>
  signal = tf.pad(signal, [[0, 0], [0, tf.mod(channels, 2)]]) # AMBYER >> <len, dim> -> <len, dim [+ 1 if dim is odd]>
  signal = tf.reshape(signal, [1, length, channels]) # AMBYER >> <1, len, dim>
  return x + signal
...
def dot_product_attention(q,
  ...
  with tf.variable_scope(
    ...
    logits = tf.matmul(q, k, transpose_b=True) # AMBYER >> q, k <batch * beam, numhead, , dim / numhead>
    if bias is not None:
      logits += bias
  ...
  with tf.variable_scope(
    ...
    if memory_antecedent is None:
      ...
      combined = common_layers.conv1d(
          ...
          1,
          name="qkv_transform") # AMBYER >> qkv_transform : head_i = Attention(QW_qi, KW_ki, VW_vi)
      q, k, v = tf.split(
    ...
    key_depth_per_head = total_key_depth // num_heads # AMBYER >> total_key|value_depth : the dimension
    q *= key_depth_per_head**-0.5
    x = dot_product_attention(
        q, k, v, bias, dropout_rate, summaries, image_shapes)
    x = combine_heads(x)
    x = common_layers.conv1d(x, output_depth, 1, name="output_transform") # AMBYER >> MultiHead(Q, K, V) = Concate(head_1, head_2, ..., head_h)W_o
    return x
